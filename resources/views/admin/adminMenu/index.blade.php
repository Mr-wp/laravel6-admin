        ...
        <div class="page-header">
            <h1>
                ...
                <span class="btn btn-sm btn-primary pull-right"
                      onclick="javascript:window.location.href = 'info'">添加</span>
            </h1>
        </div>
                ...
                <form name="myform-select" method="GET" class="form-inline">
                    <div class="form-group select-input">
                        ...
                        <div class="input-group">
                            ...
                            {!! form_select(\App\Models\AdminMenu::$statusArr,request('status'),'class="form-control" name="status"','--请选择--') !!}
                        </div>
                        <div class="input-group">
                            <div class="input-group-addon">状态</div>
                            {!! form_select(\App\Models\AdminMenu::$writeLogArr,request('write_log'),'class="form-control" name="write_log"','--请选择--') !!}
                        </div>
                        <div class="input-group">
                        ...
                        <table id="simple-table" class="table  table-bordered table-hover">
                            ...
                            <form action="" method="post" name="myform">
                                ...
                                @foreach($lists as $info)
                                    <tr>
                                        <td>{{$info['id']}}</td>
                                        <td><input type="text" name="listorder[{{$info['id']}}]"
                                                   value="{{$info['listorder']}}" width="10"></td>
                                        <td><span class="fa {{$info['icon']}}"></span> {{$info['name']}}</td>
                                        <td>{{$info['m']}}/{{$info['c']}}/{{$info['a']}}</td>
                                        <td class="hidden-480">
                                            <span class="label label-sm label-warning">{{$info['data']}}</span>
                                        </td>
                                        <td>{{\App\Models\AdminMenu::$statusArr[$info['status']]}}</td>
                                        <td>{{\App\Models\AdminMenu::$writeLogArr[$info['write_log']]}}</td>
                                        <td>
